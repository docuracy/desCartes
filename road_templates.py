...
def score_linestrings(linestrings, TEMPLATE_SAMPLE, raster_image_gray, MAX_ROAD_WIDTH, MIN_ROAD_WIDTH):
    ...
    for k, (linestring) in enumerate(linestrings):
        ...
        if (linestring.length < TEMPLATE_SAMPLE):
            linestring_attributes.append({'score': 0, 'width': 0})
            continue
        scores = []
        widths = []
        ...
        if np.isnan(mean_score) or mean_score == 0:
            ...
            attributes = {'score': int(mean_score*100), 'width': mean_width}
            linestring_attributes.append(attributes)
            # print(linestring.length, attributes)    
                            
    return linestring_attributes